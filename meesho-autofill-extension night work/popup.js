...
async function sendFillRequest(data) {
  ...
  try {
    ...
    // Check if we're on the Meesho supplier page
    ...
    log('Received response from content script', 'success');
    return response;
    
  } catch (error) {
    log('Error sending message to content script: ' + error.message, 'error');
    throw error;
  }
}
